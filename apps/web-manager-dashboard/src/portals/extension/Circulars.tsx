...
export default function CircularsPage() {
  const { t } = useLanguage()
  const mockCirculars = [
    { id: 'TRI001', title: t('Feb 2026 - Tea Price Revision'), date: '01 Feb 2026', distributed: 247, read: 188, percent: 77 },
    { id: 'TRI002', title: t('Fertilizer Subsidy Scheme 2026'), date: '15 Jan 2026', distributed: 247, read: 201, percent: 81 },
    { id: 'TRI003', title: t('Quality Standards Update - Q1 2026'), date: '10 Jan 2026', distributed: 247, read: 234, percent: 95 },
    { id: 'TRI004', title: t('Pest Alert: Blister Blight Notice'), date: '05 Dec 2025', distributed: 247, read: 247, percent: 100 },
  ];
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
         <div className="flex-1 relative">
            ...
            <input type="text" placeholder={t("Search circulars...")} className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 pl-12 pr-4 outline-none focus:ring-2 focus:ring-green-500" />
         </div>
      </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200 text-slate-900 text-[10px] font-black uppercase tracking-widest">
              <th className="px-8 py-4">{t('CIRCULAR ID')}</th>
              <th className="px-8 py-4">{t('TITLE')}</th>
              <th className="px-8 py-4 text-center">{t('DATE')}</th>
              <th className="px-8 py-4 text-center">{t('DISTRIBUTED')}</th>
              <th className="px-8 py-4 text-center">{t('READ')}</th>
              <th className="px-8 py-4 text-center">{t('READ %')}</th>
              ...
              <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-8 py-6">
                   <div className="flex items-center gap-3 justify-center">
                     ...
                     <span className="text-[10px] font-black text-slate-900 w-8">{c.percent}%</span>
                   </div>
                </td>
                <td className="px-8 py-6 text-right flex items-center justify-end gap-2">
                   <button className="bg-slate-50 border border-slate-100 px-3 py-1 rounded text-[10px] font-black text-slate-950 uppercase tracking-widest hover:bg-slate-100">{t('View')}</button>
