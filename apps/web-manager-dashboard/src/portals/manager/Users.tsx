...
export default function UsersPage() {
  ...
  const getFullRoleName = (role: string) => {
    switch (role?.toUpperCase()) {
      case 'MG': return 'Manager';
      case 'EXT': return 'Extension Officer';
      case 'ST': return 'Office Staff';
      case 'SK': return 'Store Keeper';
      case 'FT': return 'Factory Staff';
      case 'TA': return 'Transport Agent';
      case 'SH': return 'Supplier';
      default: return role;
    }
  };
  ...
  const handleCreateSH = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Failed to create Small Holder. Passbook No / Contact might exist.");
    }
  };
                ...
                <td className="px-8 py-5 text-slate-400 text-xs font-medium">{user.active}</td>
                 <td className="px-8 py-5 text-right flex items-center justify-end gap-2">
                    <button 
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black text-slate-500 uppercase tracking-widest hover:bg-slate-100 border border-transparent hover:border-slate-200 transition-all">
                       <Edit2 size={12} />
                       Profile
                    </button>
                    <button 
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-slate-50">
              <h2 className="text-lg font-bold text-slate-800">
                ...
                {step === 2 && addingRoleCategory !== 'TA' && addingRoleCategory !== 'SH' && "Create System User"}
                {step === 2 && addingRoleCategory === 'TA' && "Create Transport Agent"}
                {step === 2 && addingRoleCategory === 'SH' && "Create Supplier"}
              </h2>
            ...
            <div className="p-6 overflow-y-auto w-full">
              {step === 1 && (
                <div className="mb-2">
                   <label className="block text-sm font-bold text-slate-500 mb-3">Please select the role you wish to create:</label>
                   <select 
                      ...
                      <option value="EXT">Extension Officer</option>
                      <option value="ST">Office Staff</option>
                      <option value="FT">Factory Staff</option>
                      <option value="SK">Store Keeper</option>
                      <option value="TA">Transport Agent</option>
                      <option value="SH">Supplier (Small Holder)</option>
                   </select>

                   <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 mt-2">
                      <button type="button" onClick={closeModal} className="px-5 py-2 font-bold text-slate-500 hover:text-slate-700">Cancel</button>
                      <button type="button" onClick={() => setStep(2)} className="bg-[#2d6a4f] text-white px-8 py-2.5 rounded-xl font-bold hover:bg-[#1b4332] shadow-md transition-colors">Next</button>
                   </div>
                </div>
              )}
              ...
              {step === 2 && addingRoleCategory !== 'TA' && addingRoleCategory !== 'SH' && (
                <form onSubmit={handleCreateStaff} className="space-y-4">
                  ...
                  <div className="pt-4 mt-6 border-t border-slate-100 flex justify-end gap-3">
                    <button type="button" onClick={() => setStep(1)} className="px-5 py-2 font-bold text-slate-500 hover:text-slate-700">Back</button>
                    <button type="submit" className="bg-[#2d6a4f] text-white px-6 py-2 rounded-lg font-bold hover:bg-[#1b4332] shadow-md transition-colors">Create Staff</button>
                  </div>
                </form>
              )}
              ...
              {step === 2 && addingRoleCategory === 'TA' && (
                <form onSubmit={handleCreateTA} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Estate Processing Hub</label>
                    <select className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-sm outline-none font-medium text-slate-700" value={taData.estateId} onChange={e => setTaData({...taData, estateId: e.target.value})}>
                       <option value="">Global / Master TA</option>
                       {estates.map(es => <option key={es.estateId} value={es.estateId}>{es.name}</option>)}
                    </select>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Create PIN <span className="text-red-500">*</span></label>
                      <div className="relative">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Confirm PIN <span className="text-red-500">*</span></label>
                      <div className="relative">
                  ...
                  <div className="pt-4 mt-6 border-t border-slate-100 flex justify-end gap-3">
                    <button type="button" onClick={() => setStep(1)} className="px-5 py-2 font-bold text-slate-500 hover:text-slate-700">Back</button>
                    <button type="submit" className="bg-blue-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-blue-700 shadow-md transition-colors">Create TA</button>
                  </div>
                </form>
              )}
              ...
              {step === 2 && addingRoleCategory === 'SH' && (
                <form onSubmit={handleCreateSH} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Passbook No</label>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                       <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Land Name (Optional)</label>
                  ...
                  <div>
                     <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Field In-Charge (TA) <span className="text-red-500">*</span></label>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Create Login PIN <span className="text-red-500">*</span></label>
                      <div className="relative">
                  ...
                  <div className="pt-4 mt-6 border-t border-slate-100 flex justify-end gap-3">
                    <button type="button" onClick={() => setStep(1)} className="px-5 py-2 font-bold text-slate-500 hover:text-slate-700">Back</button>
                    <button type="submit" className="bg-orange-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-orange-700 shadow-md transition-colors">Create Supplier</button>
                  </div>
                </form>
              )}
            </div>
            ...
            <div className="pt-16 px-10 pb-10 overflow-y-auto w-full">
               <div className="flex justify-between items-start mb-8">
                  <div>
                    ...
                    <p className="text-slate-500 font-bold">{getFullRoleName(detailedUser.role)} • {detailedUser.id}</p>
               ...
               <div className="grid grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4">
                     <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">System Metadata</h3>
                     <div className="space-y-3">
                        <div>
                           <label className="text-xs font-bold text-slate-400">Current Estate</label>
                           {isEditing ? (
                        ...
                        {detailedUser.role === 'SH' && (
                           <>
                              <div>
                                 <label className="text-xs font-bold text-slate-400">Passbook No</label>
               ...
               {detailedUser.role === 'SH' && (
                  <div className="mt-8 pt-8 border-t border-slate-100 grid grid-cols-2 gap-8">
                     <div>
                        <label className="text-xs font-bold text-slate-400">Land Name</label>
                        {isEditing ? (
                     ...
                     <div>
                        <label className="text-xs font-bold text-slate-400">Total Arcs</label>
                        {isEditing ? (
                           ...
                           <p className="text-sm font-bold text-slate-700">{detailedUser.arcs} Arcs</p>
                        )}
                     </div>
                  </div>
               )}
